    ...
    return (
      <main className="min-h-screen bg-[#f6f7fb] text-slate-900">
        ...
        <section className="mx-auto max-w-[1480px] px-5 py-7 lg:px-8">
          <GenerationRecordsTable records={records} />
        </section>
      </main>
    )
  } catch (error) {
    const message = error instanceof Error ? error.message : '任务生成记录读取失败。'
      ...
      <main className="min-h-screen bg-[#f6f7fb] text-slate-900">
        ...
        <section className="mx-auto max-w-4xl px-5 py-12 lg:px-8">
          <div className="rounded-xl border border-red-200 bg-white p-7 shadow-sm">
            ...
            <h1 className="mt-2 text-2xl font-semibold text-slate-950">任务生成记录暂时不可用</h1>
...
function GenerationRecordsTable({ records }: { records: AdminGenerationRecordItem[] }) {
  return (
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 bg-slate-50/80 px-5 py-4">
        <h2 className="text-base font-semibold text-slate-950">最近 200 条任务生成记录</h2>
        <p className="mt-1 text-xs text-slate-500">
          当前显示 {records.length.toLocaleString('zh-CN')} 条记录，可从用户列回到单个用户使用记录。
        </p>
      </div>
                  ...
                  <td className="px-4 py-4">
                    ...
                    >
                      <p className="truncate font-semibold text-slate-950">
                        {record.userName || '未设置姓名'}
                      </p>
                      <p className="mt-0.5 truncate text-xs text-slate-500">{record.userEmail}</p>
