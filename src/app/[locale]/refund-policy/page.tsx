...
export function generateStaticParams() {
  return SUPPORT_PAGE_LOCALES.filter((locale) => locale !== 'en').map((locale) => ({ locale }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params
  const copy = getSupportPolicyCopy('refundPolicy', locale)
...
export default async function LocalizedRefundPolicyPage({ params }: PageProps) {
  ...
  return (
    <SupportPolicyPageContent
      locale={locale}
      copy={getSupportPolicyCopy('refundPolicy', locale)}
