...
export const metadata: Metadata = {
  ...
  description:
    'Buy Toolaze AI generation credits with one-time packages. Credits are valid for 12 months, failed generations are refunded, and commercial use is allowed.',
  robots: 'index, follow',
  alternates: {
        ...
        <section className="mx-auto max-w-6xl">
          <div className="mx-auto w-full max-w-3xl text-center">
            <h1 className="mx-auto max-w-[21rem] text-[32px] font-extrabold leading-tight tracking-tight text-slate-800 sm:max-w-3xl sm:text-[40px] md:text-[46px]">
              <span className="sm:hidden">Buy credits once. Use them when you need them</span>
              <span className="hidden sm:inline">Buy Toolaze credits once, use them when you need them</span>
            </h1>
            <p className="mx-auto mt-4 max-w-[20rem] text-base leading-7 text-slate-600 sm:max-w-2xl md:text-lg">
              Choose a credit pack for supported Toolaze AI generation tools. No subscription required.
            </p>
          </div>
          ...
          <div className="mt-10 grid min-w-0 gap-5 md:grid-cols-2 xl:grid-cols-3">
            {PRICING_PLANS.map((plan) => {
              ...
              const hasTopBadge = Boolean(bonusBadge || plan.badge)
                ...
                >
                  <div className="flex h-full min-w-0 flex-col overflow-hidden rounded-[1.7rem] bg-white transition duration-300 group-hover:bg-[#FCFBFF]">
                    <div className="relative bg-gradient-to-br from-[#F7F3FF] via-white to-[#EEF2FF] p-5 pb-5">
                      {bonusBadge ? (
                        <span className="absolute left-0 top-0 rounded-br-2xl bg-gradient-to-r from-indigo-500 via-violet-500 to-purple-600 px-4 py-1.5 text-sm font-extrabold text-white shadow-lg shadow-indigo-200/70">
                          {bonusBadge}
                        </span>
                      ) : null}

                      {plan.badge ? (
                        <span className="absolute right-0 top-0 rounded-bl-2xl bg-amber-100 px-4 py-1.5 text-sm font-extrabold text-amber-700 shadow-lg shadow-amber-100/80 ring-1 ring-amber-200">
                          {plan.badge}
                        </span>
                      ) : null}

                      <div className={`flex items-start justify-between gap-4 ${hasTopBadge ? 'pt-7' : ''}`}>
                      ...
                      <div className="mt-2 text-center">
                        {plan.bonusCredits > 0 ? (
                          <p className="inline-flex items-center justify-center rounded-full bg-white/80 px-4 py-2 text-sm font-extrabold text-slate-600 shadow-sm shadow-indigo-100 ring-1 ring-indigo-100">
                          ...
                          <p className="inline-flex items-center justify-center rounded-full bg-white/75 px-3 py-2 text-sm font-extrabold text-slate-600 shadow-sm shadow-indigo-100 ring-1 ring-indigo-100">
                    ...
                    <div className="flex flex-1 flex-col p-5 pt-4">
                      ...
                      <button
                        type="button"
                        disabled
                        className={`mt-4 rounded-full px-5 py-3 text-sm font-extrabold transition duration-300 disabled:cursor-not-allowed ${
                          isFeatured
                            ? 'bg-gradient-to-r from-indigo-500 via-violet-500 to-purple-600 text-indigo-50 shadow-lg shadow-indigo-200/80 disabled:opacity-90'
