...
const defaultCreditsPageCopy = {
  ...
  balanceAfter: 'Balance after {balance}',
  types: {
    grant: 'Bonus',
    use: 'Used',
    refund: 'Refund',
...
export default function CreditsPageClient({ initialTranslations, locale = 'en' }: CreditsPageClientProps = {}) {
  ...
  useEffect(() => {
    ...
    const loadCredits = async () => {
      ...
      setError('')
      try {
        const response = await fetch('/api/credits?limit=200', { cache: 'no-store' })
  ...
  return (
    <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-10 md:px-6 md:py-14">
      ...
      <section className="mb-6 rounded-2xl border border-slate-200 bg-white px-5 py-5">
        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-indigo-600">{copy.availableCredits}</p>
              ...
              <article key={transaction.id} className="grid gap-3 border-b border-slate-100 px-5 py-4 last:border-b-0 sm:grid-cols-[1fr_auto]">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {transaction.reason} · {formatUiText(copy.balanceAfter, { balance: transaction.balanceAfter })}
                  </p>
                </div>
