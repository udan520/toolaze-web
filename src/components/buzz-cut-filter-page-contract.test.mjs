...
test('Buzz Cut Filter reuses the male hairstyle preset library with Buzz Cut selected by default', () => {
  ...
  assert.match(aiImageToolSource, /shouldUseCustomReferenceUploader = shouldUseCombinedCustomReference \|\| shouldUseReferenceOnlyCustomMode/)
  ...
  assert.match(aiImageToolSource, /shouldUsePresetReferenceUploader = shouldRenderWorkflowTabsAboveUpload[\s\S]*activePromptPresetTab !== customPromptTabId[\s\S]*\(!hidePresetReferenceUploader \|\| inlinePresetReferenceUpload\)/)
  assert.match(aiImageToolSource, /const effectiveRequestPrompt = shouldUseCombinedCustomReference[\s\S]*shouldUseReferenceOnlyCustomMode[\s\S]*customReferencePrompt\.trim\(\)/)
  assert.match(aiImageToolSource, /const currentEffectivePrompt = shouldUseCombinedCustomReference[\s\S]*shouldUseReferenceOnlyCustomMode[\s\S]*customReferencePrompt\.trim\(\)/)
  assert.match(aiImageToolSource, /const canGenerate = Boolean\(currentEffectivePrompt\)[\s\S]*activeTab !== 'image-to-image'[\s\S]*hasCurrentReferenceImages/)
