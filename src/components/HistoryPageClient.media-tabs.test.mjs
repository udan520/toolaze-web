...
test('History page supports selecting records for batch download and delete', () => {
  ...
  assert.match(source, /\{selectionMode && item\.status === 'succeeded' && \(\s*<label[\s\S]*data-history-card-select/)
...
test('History cards expose a media-library import action only for admin users', () => {
  ...
  assert.match(source, /const payload = await response\.json\(\)\.catch\(\(\) => \(\{\}\)\)/)
  assert.match(source, /payload\.importedCount/)
  assert.match(source, /payload\.skippedCount/)
  assert.match(source, /\{isMediaLibraryAdmin && item\.status === 'succeeded' && !selectionMode && \(/)
