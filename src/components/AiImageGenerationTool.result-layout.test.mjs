...
test('reference-shaped image generation uses a compact Match Reference control and effective metadata', () => {
  ...
  assert.match(source, /data-compact-match-reference-aspect-ratio/)
  assert.match(source, /followsReferenceImageAspectRatio \? \([\s\S]*data-compact-match-reference-aspect-ratio[\s\S]*Match Reference[\s\S]*\) : \(/)
...
test('pending generations replace blob previews with stable uploaded references before task creation', () => {
  const stableReferenceUpdate = source.indexOf('inputPreview: generationInputUrls[0] ||')
  const taskCreation = source.indexOf('requestImageGenerationTask(formData, toolText)')

  assert.notEqual(stableReferenceUpdate, -1, 'pending state should receive the stable uploaded references')
  assert.notEqual(taskCreation, -1, 'generation task request should be present')
  assert.ok(stableReferenceUpdate < taskCreation, 'stable references must be persisted before task creation')
  assert.match(source, /setPendingGenerationItems\(\(prev\) => prev\.map\(\(item\) => item\.id === pendingItem\.id[\s\S]*inputPreview:\s*generationInputUrls\[0\] \|\| ''[\s\S]*inputUrls:\s*generationInputUrls/)
})
...
test('history action uses settings instead of generating immediately', () => {
  ...
  assert.match(historyApply, /setAspectRatioForMode\(\s*recreateGenerationMode,[\s\S]*getDefaultAspectRatioForModel\(item\.modelId \|\| selectedModelId, presetMode, recreateGenerationMode\)/)
  ...
  assert.match(historyApply, /handleGenerationModeChange\(recreateGenerationMode\)/)
  ...
  assert.match(failedApply, /setAspectRatioForMode\(\s*restoredMode,[\s\S]*getDefaultAspectRatioForModel\(item\.modelId, presetMode, restoredMode\)/)
  ...
  assert.match(failedApply, /const restoredMode = item\.generationMode \?\? \(inputImageUrls\.length > 0 \? 'image-to-image' : 'text-to-image'\)/)
  assert.match(failedApply, /handleGenerationModeChange\(restoredMode\)/)
...
test('history action can edit a generated image as the next reference image', () => {
  ...
  assert.match(historyEdit, /handleGenerationModeChange\('image-to-image'\)/)
...
test('pending reprompt can apply an image-only edit payload', () => {
  ...
  assert.match(promptInsert, /handleGenerationModeChange\(nextMode\)/)
})
