...
test('credits page formats transaction titles and model supplements for display', () => {
  ...
  assert.match(source, /transactionSupplement && \(/)
  assert.match(source, /\{balanceText\}/)
  assert.doesNotMatch(source, /transactionSupplement \? `\$\{transactionSupplement\} · \$\{balanceText\}` : balanceText/)
