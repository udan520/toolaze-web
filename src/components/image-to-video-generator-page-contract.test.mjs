...
test('image-to-video prompt cards send their poster image into the video generator', () => {
  ...
  assert.match(promptUseDetailSource, /const imageUrl = item\.referenceImage \|\| item\.image \|\| item\.poster/)
  assert.match(promptUseDetailSource, /imageUrl,/)
  assert.match(promptUseDetailSource, /const imageName =/)
  assert.match(promptUseDetailSource, /imageName,/)
  assert.match(promptExamplesSource, /poster\?: string/)
  assert.match(videoToolSource, /imageUrl\?: string/)
  assert.match(videoToolSource, /const promptImageUrl = String\(detail\?\.imageUrl \|\| ''\)\.trim\(\)/)
  assert.match(videoToolSource, /setActiveMode\(promptImageUrl \? 'image-to-video' : 'text-to-video'\)/)
  assert.match(videoToolSource, /setRemoteImageUrls\(promptImageUrl \? \[promptImageUrl\] : \[\]\)/)
})
