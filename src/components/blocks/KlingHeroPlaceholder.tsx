    ...
    <div className="max-w-xl mx-auto relative z-10">
      <div className="bg-white rounded-super p-2 shadow-soft border border-indigo-50">
        <div className="border-2 border-dashed border-indigo-100 rounded-[2.2rem] p-10 bg-indigo-50/20 text-center relative overflow-hidden group hover:border-indigo-300 transition-all cursor-pointer">
          ...
          <p className="text-[12px] font-bold text-indigo-400 uppercase tracking-widest">{text?.klingTagline || '4K • 6-Shot • Multilingual Audio'}</p>
          <p className="text-sm text-slate-500 mt-4">{text?.klingDescription || 'Kling 3.0 AI video generator will be available here soon.'}</p>
        </div>
      </div>
      <div className="mt-8 flex flex-wrap justify-center gap-4 text-[11px] font-bold text-indigo-900/60 uppercase tracking-widest">
        <span>{text?.native4k || '🎥 Native 4K'}</span> <span className="hidden md:block">|</span>
        <span>🎞️ 6-Shot</span> <span className="hidden md:block">|</span>
        <span>{text?.multilingualAudio || '🔊 Multilingual Audio'}</span>
      </div>
    </div>
  )
}
