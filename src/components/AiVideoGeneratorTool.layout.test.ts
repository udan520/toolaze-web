...
test('AI video generator uses the shared generation history API for completed videos', () => {
  ...
  assert.match(source, /formData\.append\('imageUrls', JSON\.stringify\(uploadedImageMedia\.generationUrls\)\)/, 'generation requests should keep using provider-safe image references')
  ...
  assert.match(source, /persistGeneratedVideoHistoryItem\(completedRequest, videoUrl, \[\.\.\.uploadedImageMedia\.historyUrls, \.\.\.uploadedMotionVideoMedia\.historyUrls\]/, 'shared history should persist every displayable reference image and motion video')
