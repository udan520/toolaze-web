...
- **板块顺序**：`performanceMetrics` 属于靠后的辅助规格/决策板块，默认放在 `sectionsOrder` 的后段，并紧贴 `faq` 上方；不要放在 How To、Prompt Examples 或其它早期操作指导板块之前。
- **相关工具媒体**：More Tools / 相关工具卡片顶部默认复用被推荐页面的顶部 Demo 图或 Demo 视频，不使用纯 icon 卡片；若目标页暂无可用 Demo 媒体，只能临时使用现有 icon 兜底，并应补齐媒体资产。
