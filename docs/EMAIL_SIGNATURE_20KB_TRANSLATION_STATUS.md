# Email Signature Optimizer 页面翻译状态报告
...
### 页面信息
- **工具**: Image Compressor
- **Slug**: `email-signature-20kb`
- **文件路径**: `src/data/{lang}/image-compression.json` (email-signature-20kb 部分)
- **菜单显示**: `in_menu: false` (不在菜单中显示)
...
使用脚本 `scripts/check-email-signature-20kb-translations.js` 检查所有语言的翻译完整性。
...
- 技术术语（如 "Canvas API", "Client-Side", "Retina"）在翻译中保持英文，符合技术文档规范
- 该页面不在菜单中显示（`in_menu: false`），但仍需要完整的翻译以支持 SEO 和直接访问
