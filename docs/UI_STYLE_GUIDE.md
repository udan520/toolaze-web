...
- 页面正文中的 H1、H2、H3 等内容标题必须是所属区块的首个视觉信息，让用户先读到主题，再读辅助说明。
- 标题上方不得放 eyebrow、kicker、badge、步骤号、分类名或纯装饰标签；步骤、时间、分类等必要辅助信息放在对应标题之后。
- Breadcrumb 和全局导航是页面定位结构，不属于内容标题上方的装饰标签，可按现有共享布局保留。
...
- 首页、AI tools、模型卡片不要用 emoji 或字母占位作为主视觉；优先使用真实 demo、工具样张或模型能力相关缩略图。同一个工具 / 模型在顶部导航下拉、首页、AI Tools Hub、相关工具推荐里的封面图，必须与该页面 Demo 图 / Demo 视频保持一致。

## 8. 图片与媒体资产

- 首页高级 AI 工具卡、首页模型卡和模型/工具落地页主视觉应复用或严格对齐对应页面 Demo 媒体；更换页面 Demo 图 / Demo 视频时，同步更新顶部导航下拉、首页和 AI Tools 里的封面图。
