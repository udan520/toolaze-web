...
if (!enPNG) {
  console.error('❌ compress-png 在英文文件中不存在')
  process.exit(1)
}
...
function isEnglish(text) {
  ...
  const englishPatterns = [
    ...
    /Is PNG compression/i,
    /Will transparency/i,
    /What file sizes/i,
    /Does the compressed PNG/i,
  ]
...
function checkTranslation(locale) {
  ...
  if (!pngData) {
    return { locale, exists: false, missing: ['compress-png'], untranslated: [] }
  }

  // 获取所有英文键
  const enKeys = getAllKeys(enPNG)
  const currentKeys = getAllKeys(pngData)
...
// 主函数
console.log('检查 compress-png 页面的多语言翻译完整性...\n')
...
results.forEach(result => {
  ...
  if (!result.exists) {
    console.log('❌ 文件不存在或缺少 compress-png 数据')
    hasIssues = true
