...
function main() {
  ...
  for (const lang of languages) {
    ...
    if (!langData || !langData[toolSlug]) {
      console.log(`语言: ${lang.toUpperCase()}\n--------------------------------------------------------------------------------\n⚠️  未找到 ${toolSlug} 的数据。`)
      hasIssues = true
      continue
    }

    const untranslated = findUntranslated(enData[toolSlug], langData[toolSlug])
